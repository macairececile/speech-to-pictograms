...
def prepare_data(
    ...
    valid_csv_file : str, optional
        Path to the Valid Common Voice .tsv file (cs)
    ...
    if valid_csv_file is None:
        valid_csv_file = data_folder + "/valid.csv"
    else:
    ...
    file_pairs = zip(
        [train_csv_file, valid_csv_file, test_csv_file],
        [save_csv_train, save_csv_valid, save_csv_test],
    )
    for tsv_file, save_csv in file_pairs:
        create_csv(
            tsv_file, save_csv, data_folder, clips_folder, accented_letters
        )
