...
def merge_data(data, src_sentence_asr, row_name):
    ids = []
    translation = []
    for i, row in data.iterrows():
        ids.append(row["clips"])
        translation.append({"fr": src_sentence_asr[int(i)], "frp": row[row_name]})
...
def get_text_and_generate_file_orfeo(folder_data_asr, model_prefix, folder_to_save, folder_input):
    ...
    for c in corpus_name:
        ...
        src_sentences_ASR = get_data_from_whisper(folder_data_asr, model_prefix, "test_"+ c)
        file = merge_data(data, src_sentences_ASR, "tgt")
...
def get_text_and_generate_file_commonvoice(folder_data_asr, model_prefix, folder_to_save, folder_input, corpus_name):
    ...
    src_sentences_ASR = get_data_from_whisper(folder_data_asr, model_prefix, "test_" + corpus_name)
    file = merge_data(data, src_sentences_ASR, "tokens")
...
if __name__ == '__main__':
    ...
    folder_data_asr = "/gpfswork/rech/czj/uef37or/ASR_S2P_data/seamlessm4t-large-v2/"
    model_prefix = "large-v2_"
    get_text_and_generate_file_orfeo(folder_data_asr, model_prefix,
                                     "/gpfswork/rech/czj/uef37or/Fine_tuning_t5/data_asr/seamless/",
                                     "/gpfswork/rech/czj/uef37or/data/orfeo/")
    get_text_and_generate_file_commonvoice(folder_data_asr, model_prefix,
                                           "/gpfswork/rech/czj/uef37or/Fine_tuning_t5/data_asr/seamless/",
                                           "/gpfswork/rech/czj/uef37or/data/commonvoice/", "commonvoice")